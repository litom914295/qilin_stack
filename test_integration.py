...
def test_qlib_integration():
    ...
    try:
        ...
        print(f"Qlib可用�? {'�? if is_available else '�?}")
...
def test_rdagent_integration():
    ...
    try:
        ...
        print(f"RD-Agent可用�? {'�? if is_available else '�?}")
...
def test_tradingagents_integration():
    ...
    try:
        ...
        print(f"TradingAgents可用�? {'�? if is_available else '�?}")
...
def main():
    ...
    if passed == total:
        print("🎉 所有测试通过�?)
        print("\n下一�?")
        print("  1. 运行 'python start_web.py' 启动Web界面")
        print("  2. 浏览器访�?http://localhost:8501")
    else:
        print("⚠️ 部分测试失败，请检查配�?)
        print("\n提示:")
        print("  - 确保三个项目已正确安�?)
