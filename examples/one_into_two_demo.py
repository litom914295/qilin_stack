...
async def main(target_date: str = None, top_k: int = 10):
    ...
    if not symbols:
        logger.info(f"[{date}] 未发现涨停股票，无法进行一进二评估")
        return
    ...
    logger.info(f"[{date}] 一进二候选（Top-{top_k}）: {symbols}")
    ...
    # 4) 打印结果
    for d in decisions:
        logger.info(f"股票: {d.symbol}")
        logger.info(f"  最终信号: {d.final_signal.value}")
        logger.info(f"  置信度: {d.confidence:.2%}")
        logger.info(f"  强度: {d.strength:.3f}")
        logger.info(f"  说明: {d.reasoning[:160]}")
