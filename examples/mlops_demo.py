...
def demo_model_registry():
    ...
    logger.info(f"✓ 已注册模型数量: {len(models)}")
    
    for model_info in models:
        logger.info(f"  - {model_info['name']}: {len(model_info['versions'])} 个版本")
    ...
    if models:
        ...
        logger.info(f"✓ 模型 {model_name} v{version} 已提升到 Production")


def demo_experiment_tracking():
    """演示实验追踪功能"""
    logger.info("="*60)
    logger.info("演示2: 实验追踪")
    logger.info("="*60)
    ...
    for n_estimators in [50, 100, 150]:
        ...
        logger.info(f"✓ n_estimators={n_estimators}, accuracy={accuracy:.4f}")
...
def demo_ab_testing():
    ...
    for variant_stat in analysis['variants']:
        logger.info(f"  {variant_stat['variant']}:")
        logger.info(f"    - 请求数: {variant_stat['requests']}")
        logger.info(f"    - 准确率: {variant_stat['accuracy']:.4f}")
    
    if analysis['winner']:
        logger.info(f"✓ 获胜者: {analysis['winner']}")
...
def demo_online_learning():
    ...
    logger.info("✓ 在线学习管道已启动")
    
    # 添加新样本
    for i in range(300, 450):
        pipeline.add_sample(X_df.iloc[i], y_s.iloc[i])
    
    logger.info(f"✓ 已添加 150 个新样本到缓冲区")
    
    # 获取缓冲区统计
    stats = pipeline.get_buffer_stats()
    logger.info(f"  - 缓冲区大小: {stats['size']}/{stats['capacity']}")
    logger.info(f"  - 利用率: {stats['utilization']:.2%}")
    ...
    if update:
        logger.info(f"✓ 模型已更新到 v{update.model_version}")
        logger.info(f"  - 触发原因: {update.trigger}")
        logger.info(f"  - 训练样本数: {update.samples_count}")
        logger.info(f"  - 更新前准确率: {update.metrics_before.get('accuracy', 'N/A')}")
        logger.info(f"  - 更新后准确率: {update.metrics_after.get('accuracy'):.4f}")
    ...
    if not history.empty:
        logger.info("更新历史:")
        logger.info("\n" + str(history[['model_version', 'trigger', 'samples_count', 'accuracy_after']]))
    
    # 停止管道
    pipeline.stop()
    logger.info("✓ 在线学习管道已停止")


def main():
    """主函数"""
    logger.info("="*60)
    logger.info("MLOps功能演示")
    logger.info("="*60)
    logger.info("请确保MLflow服务正在运行:")
    logger.info("  docker-compose up -d mlflow")
    logger.info("  或")
    logger.info("  mlflow server --host 0.0.0.0 --port 5000")
    logger.info("按Enter继续...")
    input()
    ...
    try:
        ...
        logger.info("所有演示已完成!")
        logger.info("="*60)
        logger.info("访问 http://localhost:5000 查看MLflow UI")
